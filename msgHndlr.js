...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const apiKey = 'vb2RIpSKolyaMRRb2Mqt' // apikey you can get it at https://mhankbarbar.moe
        ...
        const ownerNumber = ["6289668128700@c.us"] // replace with your whatsapp number
        const isOwner = ownerNumber.includes(sender.id)
        const isBlocked = blockNumber.includes(sender.id)
        const isNsfw = isGroupMsg ? nsfw_.includes(chat.id) : false
        const uaOverride = 'WhatsApp/2.2029.4 Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Safari/537.36'
        const isUrl = new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/gi)
        if (!isGroupMsg && command.startsWith('!')) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32mEXEC\x1b[1;37m]', time, color(msgs(command)), 'dari', color(pushname))
        if (isGroupMsg && command.startsWith('!')) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32mEXEC\x1b[1;37m]', time, color(msgs(command)), 'dari', color(pushname), 'di grub', color(formattedTitle))
        ...
        case '!sgif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    await exec(`gify ${filename} ./media/output.gif --fps=60 --scale=500:500`, async function (error, stdout, stderr) {
	    ...
	    if (isMedia) {
                try {
                    ...
                    var result = await removeBackgroundFromImageBase64({ base64img, apiKey: '9JYH9SMRMXxpgpcZz2XVuBya', size: 'auto', type: 'auto', outFile })
        ...
        case '!donasi':
        case '!donate':
            client.sendLinkWithAutoPreview(from, 'https://saweria.co/ilhamshf', donate)
            break
        case '!tts':
        ...
        case '!fb':
            ...
            break
        case '!creator':
            client.sendContact(from, '6289668128700@c.us')
            break
        case '!ig':
        ...
        case '!bc':
            ...
            for (let ids of chatz) {
                ...
                if (!cvk.isReadOnly) await client.sendText(ids, `[ DUH BOT Broadcast ]\n\n${msg}`)
            }
        ...
        case '!adminlist':
            ...
            for (let admon of groupAdmins) {
                mimin += `Berikut adalah admin di grup *${chat.contact.name}*\n\n➸ @${admon.replace(/@c.us/g, '')}\n` 
            }
        ...
        case '!mentionall':
            ...
            let hehe = `╔══✪〘 Mention semua warga ${chat.contact.name} 〙✪══\n`
